...
# appears outside `src/output/mod.rs`. The single-owner invariant is
...
# `src/output/` directory. `OutputConfig` owns no I/O handles — its print
...
if [[ -n "$matches" ]]; then
  echo "FAIL: naked println!/eprintln!/print!/eprint! outside src/output/mod.rs:" >&2
  echo "$matches" >&2
  echo "" >&2
...
echo "OK: no naked stdio macros outside src/output/mod.rs"
