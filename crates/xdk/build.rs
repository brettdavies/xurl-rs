...
//! `vendor/x-api-openapi.json` into `$OUT_DIR/auth_matrix.rs`, the legacy
//! post-vocabulary table into `$OUT_DIR/vocabulary.rs`, and the streaming
//! endpoint set into `$OUT_DIR/streaming.rs`, and emit the spec-provenance
//! env vars `src/lib.rs` exposes as consts.
//!
...
// ── Streaming endpoint set ──────────────────────────────────────────────
//
// `codegen/streaming.rs` collects every path with an operation marked
// `x-twitter-streaming`; this emits them for `src/api/endpoints.rs`.

/// Emit `$OUT_DIR/streaming.rs`: `STREAMING_PATHS`, in path order.
fn emit_streaming(spec: &serde_json::Value, spec_path: &Path) {
    let paths = streaming::derive(spec).unwrap_or_else(|e| panic!("{}: {e}", spec_path.display()));
    ...
    src.push_str("// Refresh via scripts/refresh-x-openapi.sh; `cargo build` regenerates.\n\n");
    src.push_str("pub const STREAMING_PATHS: &[&str] = &[\n");
    for path in &paths {
        writeln!(&mut src, "    {path:?},").expect("write to String never fails");
    }
    src.push_str("];\n");

    let out_path = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR")).join("streaming.rs");
    fs::write(&out_path, src)
        .unwrap_or_else(|e| panic!("cannot write {}: {e}", out_path.display()));
}
