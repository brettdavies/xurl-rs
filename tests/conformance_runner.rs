...
fn run_differential_conformance_suite() {
    ...
    if original.is_none() {
        ...
        if !std::path::Path::new(known_path).exists() {
            eprintln!("SKIP: Go xurl binary not found. Set XURL_ORIGINAL_BIN or install at {known_path}");
            return;
        }
        unsafe { std::env::set_var("XURL_ORIGINAL_BIN", known_path); }
    }
    ...
    let cases: TestCaseFile = toml::from_str(&content)
        .unwrap_or_else(|e| panic!("Failed to parse test_cases.toml: {e}"));
