...
#[test]
fn test_json_output_version_is_json() {
    // `xurl version --output json` should emit {"message":"xurl X.Y.Z"}
    ...
    assert!(parsed["message"].as_str().unwrap().contains("xurl"));
}
...
fn test_xurl_output_env_sets_json_format() {
    // XURL_OUTPUT=json should make version output JSON
    let output = Command::cargo_bin("xr")
        .unwrap()
        .args(["version"])
...
fn test_quiet_suppresses_version_output() {
    // Even with --quiet, version should still print (it's the actual response)
    let output = Command::cargo_bin("xr")
        .unwrap()
        .args(["version", "--quiet"])
        .output()
...
fn test_json_quiet_combined() {
    // --output json --quiet should still produce JSON output
    let output = Command::cargo_bin("xr")
        .unwrap()
        .args(["version", "--output", "json", "--quiet"])
        .output()
        .unwrap();
